...
function deploy(abi, bytecode, signer, arbiterAddress, beneficiaryAddress) {
    // Create a ContractFactory with the provided abi, bytecode and signer
    const factory = new ethers.ContractFactory(abi, bytecode, signer);
    
    // Deploy the contract, passing the constructor arguments and specifying a value of 1 Ether
    const contract = factory.deploy(arbiterAddress, beneficiaryAddress, {
        value: ethers.utils.parseEther("1.0")  // Send 1 Ether to the contract during deployment
    });

    // Return the deployment promise
    return contract;
}
